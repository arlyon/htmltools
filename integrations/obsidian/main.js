...
class Tokenizer {
  ...
  _err(code, cpOffset = 0) {
    var _a, _b;
    (_b = (_a = this.handler).onParseError) === null || _b === undefined || _b.call(_a, this.preprocessor.getError(code, cpOffset));
  }
...
class Parser {
  ...
  onItemPush(node, tid, isTop) {
    var _a, _b;
    (_b = (_a = this.treeAdapter).onItemPush) === null || _b === undefined || _b.call(_a, node);
  ...
  onItemPop(node, isTop) {
    ...
    (_b = (_a = this.treeAdapter).onItemPop) === null || _b === undefined || _b.call(_a, node, this.openElements.current);
    if (isTop) {
      let current;
      let currentTagId;
  ...
  _attachElementToTree(element, location) {
    ...
    if (this._shouldFosterParentOnInsertion()) {
      ...
      this.treeAdapter.appendChild(parent !== null && parent !== undefined ? parent : this.document, element);
    }
  }
...
function endTagInText(p, token) {
  ...
  if (token.tagID === TAG_ID.SCRIPT) {
    (_a = p.scriptHandler) === null || _a === undefined || _a.call(p, p.openElements.current);
  }
  p.openElements.pop();
...
// src/htmltool-signature.ts
var MANIFEST_MIME = "application/htmltool+json";
function inspectHtmlTool(source) {
  const manifests = [];
...
var DEFAULT_CHECK_TIMEOUT_MS = 5000;
var MAX_DIAGNOSTIC_LENGTH = 16000;
function startToolServer(options) {
  ...
  const child = spawnProcess(executable, [options.toolPath, "--no-open", "--port", "0"], {
    cwd: import_node_path.dirname(options.toolPath),
    env: {
      ...process.env,
...
class ToolStartup {
  ...
  attach() {
    this.timeout = setTimeout(this.onTimeout, this.options.startupTimeoutMs ?? DEFAULT_STARTUP_TIMEOUT_MS);
  ...
  onError = (error) => {
    ...
    this.fail(new Error(`Could not start HTMLTool: ${error.message}`), "already-stopped");
  };
  onExit = (code, signal) => {
    if (this.state === "starting") {
      const detail = this.stderr.trim() || exitDescription(code, signal);
      this.fail(new Error(`HTMLTool exited before startup: ${detail}`), "already-stopped");
      return;
    }
    if (this.state === "running" && !this.stopping) {
      this.reportUnexpectedExit(`HTMLTool stopped unexpectedly (${exitDescription(code, signal)}).`);
    }
  };
  onAbort = () => {
  ...
  onTimeout = () => {
    this.fail(new Error(`HTMLTool did not report a UI URL within ${this.options.startupTimeoutMs ?? DEFAULT_STARTUP_TIMEOUT_MS} ms.`), "terminate");
  };
  stop = () => {
    if (this.stopping)
      return;
  ...
  fail(error, processDisposition) {
    if (this.state !== "starting")
      return;
    this.state = "failed";
    this.cleanupAllListeners();
    if (processDisposition === "terminate")
      this.child.kill();
    this.reject?.(error);
  }
  ...
  reportUnexpectedExit(message) {
    if (this.unexpectedExitReported)
      return;
    this.unexpectedExitReported = true;
    this.options.onUnexpectedExit?.(message);
  }
}
async function checkExecutable(executable, spawnProcess = nodeSpawn, timeoutMs = DEFAULT_CHECK_TIMEOUT_MS) {
...
function requiredExecutable(executable) {
  const value = executable.trim();
  if (!value)
    throw new Error("Configure the HTMLTool executable path first.");
  return value;
}
function appendDiagnostic(current, next) {
...
function nodeSpawn(command, args, options) {
  const { spawn } = require("node:child_process");
  return spawn(command, args, options);
}

// src/view-renderer.ts
function renderTool(contentEl, name, url) {
  prepare(contentEl);
...
function renderStatus(contentEl, title, detail) {
  prepare(contentEl);
...
function renderError(contentEl, title, detail) {
  prepare(contentEl);
...
function renderPlainHtml(contentEl, fileName, source) {
  prepare(contentEl);
...
function prepare(contentEl) {
  contentEl.empty();
  contentEl.addClass("htmltool-view");
}

// src/htmltool-view.ts
var VIEW_TYPE_HTMLTOOL = "htmltool-view";

class HtmlToolView extends import_obsidian.FileView {
  plugin;
  runningServer;
  startupAbort;
  loadToken = 0;
  ...
  async onLoadFile(file) {
    ...
    if (inspection.kind === "not-tool") {
      ...
      this.registerDomEvent(openButton, "click", () => {
        this.openInDefaultApp(file);
      });
      return;
    }
    if (inspection.kind === "invalid") {
      renderError(this.contentEl, "Invalid HTMLTool manifest", inspection.message);
      return;
    }
    const adapter = this.app.vault.adapter;
    if (!(adapter instanceof import_obsidian.FileSystemAdapter)) {
      renderError(this.contentEl, "Local vault required", "HTMLTool can only run files from a desktop filesystem vault.");
      return;
    }
    ...
    try {
      ...
      if (token !== this.loadToken)
        return;
      this.startupAbort = undefined;
      renderError(this.contentEl, "Could not start HTMLTool", errorMessage(error));
    }
  }
  ...
  async openInDefaultApp(file) {
    const adapter = this.app.vault.adapter;
    if (!(adapter instanceof import_obsidian.FileSystemAdapter)) {
      new import_obsidian.Notice("This file is not stored in a local filesystem vault.");
      return;
    }
    try {
      const { shell } = (()=>{throw new Error("Cannot require module "+"electron");})();
      const message = await shell.openPath(import_node_path2.join(adapter.getBasePath(), file.path));
      if (message)
        throw new Error(message);
    } catch (error) {
      new import_obsidian.Notice(`Could not open file: ${errorMessage(error)}`);
    }
  }
}
...
class HtmlToolSettingTab extends import_obsidian2.PluginSettingTab {
  htmlToolPlugin;
  constructor(app, htmlToolPlugin) {
    super(app, htmlToolPlugin);
    this.htmlToolPlugin = htmlToolPlugin;
  }
  display() {
    this.containerEl.empty();
    this.containerEl.createEl("h2", { text: "HTMLTool" });
    new import_obsidian2.Setting(this.containerEl).setName("Executable").setDesc("Command name or absolute path for the installed HTMLTool executable.").addText((text) => {
      text.setPlaceholder("htmltool").setValue(this.htmlToolPlugin.settings.executablePath).onChange(async (value) => {
      ...
      button.setButtonText("Test").onClick(async () => {
        ...
        try {
          ...
          new import_obsidian2.Notice("HTMLTool executable is available.");
        } catch (error) {
...
class HtmlToolPlugin extends import_obsidian3.Plugin {
  settings = DEFAULT_SETTINGS;
  async onload() {
    await this.loadSettings();
    this.registerView(VIEW_TYPE_HTMLTOOL, (leaf) => new HtmlToolView(leaf, this));
